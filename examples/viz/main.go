...
const usage = `viz - Data visualization example with SVG and terminal output
...
Examples:
  ...
  go run main.go -type heatmap -format terminal -data data/heatmap.json

  # SVG line graph from stdin
  cat data/linegraph.json | go run main.go -type line-graph -format svg > output.svg

  # Terminal bar chart with custom theme
  go run main.go -type bar-chart -data data/barchart.json -theme midnight
`
